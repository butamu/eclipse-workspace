...
public class DirectedGraph {
	...
	public DirectedGraph() {
		...
		queue = new Queue(maxVertices);//create an empty queue for BSF
		stack = new Stack(maxVertices); //create an empty stack for DFS
	...
	public void doTopo() {//display vertices in topological order
		int oldnVertices = nVertices;//save the original no of vertices
		int vIndex;
		char vLabel;
		...
		while(nVertices>0) {
			...
			if(vIndex == -1) {
				System.out.println("Error: Cycles detected in graph!");
				return;
			}
			vLabel = vertexList[vIndex].label;
			sortedArray[nVertices-1] = vLabel;
			deleteVertex(vLabel);
			
		}
		
		//display the sorted vertices
		for(int i= 0;i<oldnVertices;i++)
